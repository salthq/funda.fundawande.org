...
/**
 * FundaWande Question Class
 *
 * All functionality pertaining to the custom question functionality of Funda Wande.
 *
 * @package Core
 * @author Pango
 *
...
class FundaWande_Question {
    ...
    public function __construct() {
        ...
        add_filter('sensei_question_types', array($this, 'sensei_question_types'), 10, 1);
        add_filter('sensei_autogradable_question_types', array($this, 'sensei_autogradable_question_types'), 10, 1);
        add_filter('sensei_grade_question_auto', array($this, 'sensei_grade_question_auto'), 10, 4);
        add_filter('quiz_panel_question_field', array($this, 'quiz_panel_question_field'), 10, 5);

    }
    
    public function sensei_question_types($types)
    {
    ...
    public function sensei_autogradable_question_types($types)
    {
    ...
    public function sensei_grade_question_auto($question_grade, $question_id, $question_type, $answer)
    {
            ...
            case 'drag-and-drop-non-sequential':
                ...
                if (!is_array($answer) || count($answer) !== count($right_answer)) {
                    ...
                    foreach ($right_answer as $index => $parts) {
                        $parts = explode('-', $parts);
                        $hash0 = $this->getImageHash($parts[0]);
                        $hash1 = $this->getImageHash($parts[1]);
            ...
            case 'drag-and-drop-sequential':
                ...
                if (!is_array($answer) || count($answer) !== count($right_answer)) {
                    ...
                    foreach ($right_answer as $index => $imageId) {
                        $hash = $this->getImageHash($imageId);
    ...
    public function quiz_panel_question_field($html, $that, $question_type, $question_id, $question_counter)
    {
        $right_answer = '';
        $wrong_answers = array();
        $answer_order_string = '';
        ...
        switch ($question_type) {
            case 'multiple-choice-with-images':
                ...
                for ($i = 0; $i < $total_right; $i++) {
                    ...
                    $right_answer = '
                        ...
                        <div class="_float-left">' . $this->getUploadImageButton(null, 'Upload option image', $right_answers[$i]) . '</div>
                ...
                foreach ($wrong_answers as $i => $answer) {
                    ...
                    $wrong_answer = '
                        ...
                        <div>' . $this->getUploadImageButton(null, 'Upload option image', $answer) . '</div>
            ...
            case 'drag-and-drop-non-sequential':
                ...
                for ($i = 0; $i < $total_right; $i++) {
                    ...
                    $right_answer = '
                        <label class="answer" for="question_' . esc_attr($question_counter) . '_right_answer_' . $i . '">
                            <div class="_float-left">' . $this->getUploadImageButton(0, 'Add option image', $right_answers[$i]) . '</div>
                            <div class="_float-right">' . $this->getUploadImageButton(1, 'Add destination image', $right_answers[$i]) . '</div>
                            <input rel="' . esc_attr($right_answer_id) . '" type="text" id="question_' . esc_attr($question_counter) . '_right_answer_' . esc_attr($i) . '" name="question_right_answers[]" value="' . esc_attr($right_answers[$i]) . '" size="25" class="question_answer widefat" /> 
                            <a class="remove_answer_option"></a>
                        </label>
                    ';
    ...
    public function getUploadImageButton($dataPart, $text, $imageId)
    {
        if ($imageId) {
            if ($dataPart !== null) {
    ...
    public function getImageHash($imageId)
    {
        return md5(wp_get_attachment_url($imageId));
    }

} // end FundaWande_Question
