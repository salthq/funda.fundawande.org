 ...
 */
class PMUI_Model_Record extends PMUI_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see PMUI_Model::getBy()
	 * @return PMUI_Model_Record
	 */
	 ...
	 * @return PMUI_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMUI_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof PMUI_Model_List ? $related->convertRecords() : $related;
	}
